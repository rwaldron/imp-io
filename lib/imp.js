...
function Imp(opts) {
  ...
  awaiting.push(
    new Promise(function(resolve) {
      writeImmediate(state.agent, "payload", [255], function() {
        resolve();
      });
    })
  );
  ...
  Promise.all(awaiting).then(function() {
    this.isReady = true;
    this.emit("ready");
    temporal.loop( 250, function() {

      if (message !== "") {
        writeImmediate(state.agent, "payload", message);
...
Imp.prototype.analogRead = function(pin, handler) {
  ...
  write(state.agent, "report", [REPORT_ANALOG, pin]);
...
Imp.prototype.digitalRead = function(pin, handler) {
  ...
  write(state.agent, "report", [REPORT_DIGITAL, pin]);
...
function Pin(opts) {
  ...
  Object.defineProperties(this, {
    ...
    mode: {
      ...
      set: function(mode) {
        write(state.agent, "payload", [PIN_MODE, state.addr, mode]);
        state.mode = mode;
      }
    },
    isPwm: {
...
Pin.prototype.write = function(value) {
  ...
  write(state.agent, "payload", [command, state.addr].concat(data));
};

function sendMessage( agent, type, data, callback) {
  writeImmediate(agent, type, data, callback);
}
...
function writeImmediate(agent, type, data, callback) {
