...
function Imp(opts) {
  ...
  Promise.all(awaiting).then(function() {
    ...
    temporal.loop( 10, function() {
      ...
      if (locked && this.calledAt > unlockAt) {
        locked = false;
      }

      ["payload", "report"].forEach( function(mType) {
        if (message[mType] !== "" && !(locked)) {
          write(state.agent, mType, message.payload);
          message[mType] = "";
          locked = true;
...
Imp.prototype.analogRead = function(pin, handler) {
  ...
  enqueue(state.agent, "report", [REPORT_ANALOG, pin]);
